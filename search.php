    ...
    post.description LIKE '%{$search_term}%'
        ORDER BY post.post_id DESC LIMIT {$offset},{$limit}";
                  ...
                  if(mysqli_num_rows($result1) > 0){
                    ...
                    if($page > 1){
                        echo '<a class="pages" href="search.php?search='. $search_term .'&page='.($page - 1).'"><i class="fa fa-chevron-left"></i></a>';
                    };
                    for($i = 1;$i <= $total_page;$i++){
                        ...
                        echo '<a class="pages '.$active.'" href="search.php?search='. $search_term .'&page='.$i.'">'.$i.'</a>';
                    }
                    if($page < $total_page){
                        echo '<a class="pages" href="search.php?search='. $search_term .'&page='.($page + 1) .'"><i class="fa fa-chevron-right"></i></a>';
                    };
                    echo '</div>';
                  }
                  ?>
